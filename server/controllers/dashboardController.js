...
exports.getUserDashboard = async (req, res) => {
  try {
    ...
    // Determine how many gigs this user has posted (for Client or Freelancer as per request)
    const Gig = mongoose.models.Gig || require("../models/Gig");
    const gigsPosted = await Gig.countDocuments({ createdBy: userId });

    const myGigs = await Gig.find({ createdBy: userId }, '_id');
    const myGigIds = myGigs.map(g => g._id);

    const proposalsReceived = await Proposal.countDocuments({ gig: { $in: myGigIds } });
    const acceptedProjects = await Proposal.countDocuments({ gig: { $in: myGigIds }, status: { $in: ["Accepted", "FundReleased"] } });
