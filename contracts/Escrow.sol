...
contract Escrow {
    ...
    /**
     * @dev Refunds the escrowed funds back to the client
     * @notice Can be called by client or freelancer (e.g. on proposal rejection)
     */
    function refundFunds() external {
        // Check: only client or freelancer can trigger refund
        ...
        // Interaction: send funds back to client
        uint256 refundAmount = address(this).balance;
        (bool sent, ) = payable(i_client).call{value: refundAmount}("");
        if (!sent) {
            revert Escrow__EtherTransferFailed();
        }
        
        emit FundsRefunded(i_client, refundAmount);
    }
