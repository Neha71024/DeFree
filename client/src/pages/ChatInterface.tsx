  ...
  const handleAddMember = async (userIdToAdd: string) => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/groups/${groupId}/add-member`,
        { adminId: user?._id, userIdToAdd }
      );
      if (res.status === 200) {
          ...
          {groupDetails?.admin === user?._id && (
            <Dialog open={addMemberDialogOpen} onOpenChange={setAddMemberDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="hidden sm:flex text-black">
              ...
              <DialogContent className="text-black">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Select a user to invite to this group.
                  </DialogDescription>
                </DialogHeader>
                <ScrollArea className="h-64 mt-4 pr-4">
                  <div className="space-y-2">
                    {allUsers
                      .filter(u => u._id !== user._id && !groupDetails?.members?.includes(u._id) && !groupDetails?.members?.some((m: any) => m._id === u._id))
                      .map(u => (
                        <div key={u._id} className="flex justify-between items-center p-2 border rounded-md bg-white">
                          <div>
                    ...
                    {allUsers.filter(u => u._id !== user._id && !groupDetails?.members?.includes(u._id) && !groupDetails?.members?.some((m: any) => m._id === u._id)).length === 0 && (
                      <p className="text-sm text-center text-muted-foreground mt-4">No eligible users found.</p>
                    )}
                  </div>
                </ScrollArea>
              </DialogContent>
              ...
              return (
                <React.Fragment key={idx}>
                  ...
                  >
                    ...
                    <Card
                      className={`max-w-sm p-3 px-4 ${isOwn
                    ...
                    >
                      ...
                      <div>{msg.text}</div>
                      <div
                        className={`text-[10px] mt-1 ${isOwn ? "text-white/80" : "text-gray-500"
                          }`}
                      >
