...
export const parseEscrowError = (error: any): string => {
  if (error?.reason) {
    ...
    } else if (error.reason.includes("Escrow__NotParticipant")) {
      return "Only the client or freelancer can perform this action.";
    } else if (error.reason.includes("Escrow__FundsAlreadyReleased")) {
      return "Funds have already been released or refunded.";
