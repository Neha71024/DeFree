...
import DéFreeLogo from '@/components/ui/DéFreeLogo';
...
const Navbar = () => {
  ...
  const { account, connect, isConnected, isLoading: isWeb3Loading } = useWeb3();
  const navigate = useNavigate();
  const location = useLocation();

  // Freelancers get "Proposals" to review incoming apps. Clients get "Proposals" to manage sent ones.
  const navigationItems = [
    ...navItemsBase,
    { 
      name: 'Proposals', 
      href: user?.role === 'client' ? '/client-proposals' : '/proposals-inbox', 
      icon: Inbox 
    }
  ];
  ...
  const handleWallet = async () => {
    if (!account) {
      ...
      toast({ title: 'Wallet Connected', description: `Connected to ${account}` });
    }
  };
  ...
  const WalletButton = () => (
    ...
    >
      ...
      {account ? `${account.slice(0, 6)}...${account.slice(-4)}` : (isWeb3Loading ? 'Checking...' : 'Connect Wallet')}
    </Button>
  );
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="pl-1 sm:pl-14">
              <DéFreeLogo className="h-10 w-auto" />
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              {isAuthenticated && (
                <Button 
                  variant="outline" 
                  size="icon" 
                  onClick={() => { navigate('/community'); toast({ title: 'Community', description: 'Opening your chats' }); }}
